 ...
 */
module.exports = function makeQueryHandler( query, requiredParams, config ) {
  return function() {
    ...
    var params = _.reduce( args, function( memo, value, index ) {
      ...
      // Values are usually strings or numbers, but they may also be an array
      // of strings (relevant for values like predictionsbyroutes' "routes"
      // parameter); if they are arrays, join the values with commas.
    ...
    // console.log(config.api.root + query + queryString);
